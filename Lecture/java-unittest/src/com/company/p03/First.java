 ...
 *     - ex) main 메소드에서 이것저것 프린트 해보고 동작시켜 보기
 *   - 테스트는 스스로 검증하며, 테스트를 준비하는 것도 스스로 한다.
 *   - Continuous Integration (CI) 도구를 활용하기 위해 이것이 꼭 이루어 져야 함.
 *     - 코드가 통합될 때마다 자동으로 모든 테스트를 수행하고, 상태를 점검하는 프레임워크
 * - Timely: 적시의, 제때
 *   - 변화하는 코드는 테스트 코드를 항상, 꾸준히 작성해야 한다.
 *   - 리뷰 시스템을 통해서, 혹은 CI 도구를 이용해서 테스트 코드 작성을 강제하기도 함. -> Peer Pressure(동료 압박)
 *     - 큰 결함 없이 기존의 잘 동작하는 (변경 예정이 없는) 코드 보다는, 말썽을 일으키고, 계속해서 변하고 있는 동적인 코드에 대한 테스트를 먼저 작성
 */
public class First {
